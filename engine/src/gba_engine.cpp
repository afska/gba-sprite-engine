...
void GBAEngine::onVBlank() {
  ...
  // GBA IRQs seem eager to crash or eat up CPU. Get in, disable stuff, work,
  // enable, get out!
  stopOnVBlank();
  ...
  if ((REG_IF & INTERRUPT_VBLANK) == INTERRUPT_VBLANK) {
    ...
    if (GBAEngine::activeChannelB) {
      if (GBAEngine::activeChannelB->done()) {
        ...
        GBAEngine::activeChannelB = nullptr;  // never delete, let unique_ptr do
                                              // that, known to flip here
      } else {
...
void GBAEngine::enqueueSound(const s8* data,
                             int totalSamples,
                             int sampleRate,
  ...
  REG_SNDDSCNT |=
      control->getControlFlags();  // output to both sides, reset fifo
  REG_SNDSTAT = SSTAT_ENABLE;      // enable all sound
  u16 ticksPerSample =
      CLOCK / sampleRate;  // divide the clock (ticks/second) by the sample rate
                           // (samples/second)
...
void GBAEngine::enableTimer0AndVBlank() {
  REG_TM0CNT = TM_ENABLE | TM_FREQ_1;  // enable timer - dma auto-syncs to this
                                       // thanks to DMA_SYNC_TO_TIMER
  startOnVBlank();
}
...
GBAEngine::GBAEngine() {
  ...
  // setup interrupt control flags for vblank IRQing (started only when sound
  // played)
