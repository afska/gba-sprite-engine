...
AffineSprite::AffineSprite(const AffineSprite& other)
    : Sprite(other), affIndex(other.affIndex) {}

AffineSprite::AffineSprite(const void* imgData,
                           int imgSize,
                           int xC,
                           int yC,
                           SpriteSize spriteSize)
    : Sprite(imgData, imgSize, xC, yC, spriteSize), affIndex(0) {}
...
void AffineSprite::setTransformationMatrix(OBJ_AFFINE* matrix) {
  // Only copy over affine related pX properties, watch out with OAM
  // overlapping!
...
void AffineSprite::rebuildOamAttr1ForAffineIndex() {
  oam.attr1 = this->x | ATTR1_AFF_ID(affIndex) | (HORIZONTAL_FLIP_FLAG << 12) |
              (VERTICAL_FLIP_FLAG << 13) | (this->size_bits << 14);
}

void AffineSprite::buildOam(int tileIndex) {
  Sprite::buildOam(tileIndex);
  oam.attr0 = ATTR0_Y(this->y) | ATTR0_MODE(1) | (GFX_MODE << 10) |
